<template>
    <div class="card-form">
        ...
        <div class="card-form__inner">
            ...
            <!--            <button class="card-form__button" v-on:click="validateForm">{{ $t('cardForm.submit') }}</button>-->
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            invalidCard() {
                let cardNumberCharacterCount = this.formData.cardNumberNotMask.length
                if (cardNumberCharacterCount == 0) {
                    this.$alert(this.$t('cardForm.invalidCardNumber'), "", "error");
                    return false
                }
                ...
                this.formData.cardNumberNotMask = this.formData.cardNumberNotMask.replace(" ", "")
                this.formData.cardNumberNotMask = this.formData.cardNumberNotMask.replace(" ", "")
                this.formData.cardNumberNotMask = this.formData.cardNumberNotMask.replace(" ", "")
                this.formData.cardNumberNotMask = this.formData.cardNumberNotMask.replace(" ", "")
                return true
            ...
            showValidationMessage(message) {
                this.$alert(message + ' ' + this.$t('validation_message'), "", "error");
            },
            validateForm() {

                if (this.invalidCard()) {
                    if (this.formData.cardName.length == 0) {
                        this.showValidationMessage(this.$t('cardForm.cardName'))
                        return
                    }

                    if (this.formData.cardMonth < 1 || this.formData.cardYear < 1) {
                        this.showValidationMessage(this.$t('cardForm.expirationDate'))
                        return
                    }

                    if (this.formData.cardCvv < 1) {
                        this.showValidationMessage(this.$t('cardForm.CVV'))
                        return
                    }

                    this.$emit('can-pay', this.formData)
                }
            },
            clearForm() {
